...
Keep reusable configuration in the source checkout, private configuration and runtime state in the active Agent OS home, and provider/product state in the provider that owns it. Never invent a local API for a Codex Scheduled task or silently grant external write authority.

## Workflow

1. Resolve the active Agent OS source and private home. Run `bin/agent-os doctor --integrations --json` from the source checkout and report core failures separately from optional checks.
...
5. Preview the exact local config mutation:
...
10. Re-run `bin/agent-os doctor --integrations`. Treat runtime turn files as historical evidence only; `/hooks` review plus a fresh task are required to prove current Task Bridge pickup, and Codex Scheduled remains the authority for the automation's enabled state.
...
- Do not expose Slack channel IDs, provider identities, task content, or runtime state in the source checkout.
