...
- For an actionable named-channel signal, attach or refresh its Slack channel label even when no project is registered. The label remains on the outcome after stable project attribution. It supports display and filtering only; the separately registered stable channel mapping owns attribution.
