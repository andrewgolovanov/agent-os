...
```bash
codex plugin marketplace add andrewgolovanov/agent-os --ref v0.7.2
codex plugin add agent-os@agent-os
```

For a later release, replace `v0.7.2` with the latest published `vN.N.N` tag
...
- `dist/release/AgentOS-0.7.2-macOS.zip`;
- `dist/release/AgentOS-0.7.2-macOS.zip.sha256`;
...
to [Agent OS v0.7.2](https://github.com/andrewgolovanov/agent-os/releases/tag/v0.7.2):

```bash
shasum -a 256 -c AgentOS-0.7.2-macOS.zip.sha256
```
