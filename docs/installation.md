...
For a later release, replace `v0.4.1` with the latest published `vN.N.N` tag
...
- `dist/release/AgentOS-0.4.1-macOS.zip`;
- `dist/release/AgentOS-0.4.1-macOS.zip.sha256`;
...
to [Agent OS v0.4.1](https://github.com/andrewgolovanov/agent-os/releases/tag/v0.4.1):

```bash
shasum -a 256 -c AgentOS-0.4.1-macOS.zip.sha256
```
