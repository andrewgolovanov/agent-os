...
For a later release, replace `v0.4.0` with the latest published `vN.N.N` tag
...
- `dist/release/AgentOS-0.4.0-macOS.zip`;
- `dist/release/AgentOS-0.4.0-macOS.zip.sha256`;
...
to [Agent OS v0.4.0](https://github.com/andrewgolovanov/agent-os/releases/tag/v0.4.0):

```bash
shasum -a 256 -c AgentOS-0.4.0-macOS.zip.sha256
```
