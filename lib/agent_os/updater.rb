  ...
  class Updater
    ...
    def inspect_source
      unless git_repository?
        return unavailable_source("source checkout is not a Git repository")
      end
    ...
    def plugin_manifest_version
      ...
      raise ArgumentError, "missing Agent OS plugin manifest: #{path}" unless File.file?(path)
    ...
    def unavailable_plugin(reason)
      {
        ...
        source_version: File.file?(File.join(@source, "plugins", "agent-os", ".codex-plugin", "plugin.json")) ? plugin_manifest_version : nil,
        refresh_required: false,
        updated: false,
        action: "none",
        reason: reason
